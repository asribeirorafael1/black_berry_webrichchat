 ...
 * @memberof Support.Auth.GoogleAuthManager
 ...
 * BBM Enterprise server and another access token for using Firebase API.
 * 
 ...
 *
 * {@link https://developer.blackberry.com/files/bbm-enterprise/documents/guide/html/googleSignInIdentityManagement.html}
 *
 * @param {Support.Auth.GoogleAuthManager.AuthConfig} appAuthConfig
 * The configuration required to authenticate with the Google auth service with
 * a scope for the BBM Enterprise server.
 ...
 */
function GoogleAuthManager(appAuthConfig) {

  //#region Initialization

  const m_nonceHashKey = 'google_authentication_nonce_hash';
  const m_authConfig = appAuthConfig;
  const m_localUserInfo = {};
  let m_idTokenInfo;
  let m_loginHint;
  let m_appAccessTokenInfo;
  let m_contactsAccessTokenInfo;
  ...
  this.authenticate = function() {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        if (parsedToken) {
          if (!parsedToken.sub) {
            throw new Error('Missing sub field in JWT token');
          }
          ...
          m_loginHint = parsedToken.sub;
          m_localUserInfo.displayName =
            parsedToken.name || parsedToken.preferred_username;
          m_localUserInfo.userId = parsedToken.sub;
          m_localUserInfo.avatarUrl = parsedToken.picture;
          m_localUserInfo.email = parsedToken.email;
          resolve(m_localUserInfo);
        }
        else {
          reject(new Error('Failed to parse id token'));
        }
      }
      catch(error) {
        reject(error);
  ...
  this.getBbmSdkToken = function(isForce = false) {
    if (!isForce && m_appAccessTokenInfo 
      && m_contactsAccessTokenInfo.expires_at > Date.now()) {
      return Promise.resolve(m_appAccessTokenInfo.access_token);
    }
    else {
      return requestAccessToken('profile')
      .then(accessToken => {
  ...
  this.getUserManagerToken = function(isForce = false) {
    if (!isForce && m_contactsAccessTokenInfo
      && m_contactsAccessTokenInfo.expires_at > Date.now()) {
      return Promise.resolve(m_contactsAccessTokenInfo.access_token);
    }
    else {
      const scopes = m_authConfig.scope;
      if (!scopes.includes('https://www.googleapis.com/auth/contacts')) {
        return Promise.reject(
          'https://www.googleapis.com/auth/contacts scope is missing');
      }

      return requestAccessToken('profile '
      + 'https://www.googleapis.com/auth/contacts')
      .then(accessTokenInfo => {
        m_contactsAccessTokenInfo = accessTokenInfo;
        return m_contactsAccessTokenInfo.access_token;
      })
      .catch(error => {
        console.error('Failed to get Google contacts access token.'
          + `Error: ${error}`);
        // At this point we are unable to get access token anymore. Request new
        // id token. User will be redirected to the authentication page.
        m_idTokenInfo = undefined;
...
const AuthenticationManager = GoogleAuthManager;
